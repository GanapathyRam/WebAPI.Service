...
{
    public class JigsMasterSelectCommand : SsDbCommand
    {
        public GetJigsMasterQM Execute()
        {
            var response = new GetJigsMasterQM();
            using (var sqlCommand = CreateCommand())
            {
                sqlCommand.Connection = Connection;
                sqlCommand.CommandText = "[dbo].[uspGetJigsMaster]";
                ...
                {
                    response.GetJigsMasterList = reader.ToList<JigsMasterModel>();
                }
