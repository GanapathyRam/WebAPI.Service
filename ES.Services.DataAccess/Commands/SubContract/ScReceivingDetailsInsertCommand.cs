...
{
    public class ScReceivingDetailsInsertCommand : SsDbCommand
    {
        public void Execute(string ScNumber, string WoNumber, decimal WoSerial, decimal PartCode)
        {
            using (var sqlCommand = CreateCommand())
            {
                ...
                sqlCommand.CommandText = "[dbo].[uspAddScReceivingDetails]";
                sqlCommand.CommandType = CommandType.StoredProcedure;
                sqlCommand.Parameters.Add(AddParameter("@ScReceivingDCNumber", SsDbType.VarChar, ParameterDirection.Input, ScNumber));
